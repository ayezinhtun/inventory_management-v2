    ...
    try {
      ...
      let compQuery = supabase
        ...
        .select(
          ...
          'id, name, component_type_id, region_id, status, created_at'
      ...
      const totalQuantity    = totalComponents; // Each component is 1 unit

      const totalAssetValue  = 0; // No price tracking in components table

      const brokenComponents = comps.filter((c: any) => c.status === 'broken').length;

      const lowStock: any[] = []; // No stock tracking in components table
      ...
      const typeAcc: Record<string, { name: string; count: number }> = {};
      ...
      comps.forEach((c: any) => {
        ...
        if (!tid) {
          ...
          typeAcc['__none'] ??= { name: 'Unassigned', count: 0 };

          typeAcc['__none'].count++;

          return;

        }

        typeAcc[tid] ??= { name: types.find((t: any) => t.id === tid)?.type_name ?? 'Unknown', count: 0 };
      ...
      const regionAcc: Record<string, { name: string; count: number }> = {};

      regions.forEach((r: any) => { regionAcc[r.id] = { name: r.name, count: 0 }; });
      ...
      const componentsByRegion = Object.values(regionAcc)
        ...
        .map((r) => ({ region: r.name, count: r.count, quantity: r.count }))
