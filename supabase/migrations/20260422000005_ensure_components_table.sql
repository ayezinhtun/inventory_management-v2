-- =============================================================
-- SAFETY NET: ensure the components table (and all supporting
-- structures from previous migrations) actually exist in the DB.
-- All statements use CREATE/ALTER IF NOT EXISTS so this is fully
-- idempotent and safe to run even if migration 003 was already applied.
-- =============================================================
...
CREATE TABLE IF NOT EXISTS public.components (
  ...
  installed_in_device_id uuid,
  device_slot           text          NOT NULL DEFAULT '',
...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename='components' AND policyname='components_read') THEN
    CREATE POLICY "components_read" ON public.components FOR SELECT TO authenticated USING (NOT is_deleted);
  END IF;
END $$;

DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename='components' AND policyname='components_insert') THEN
    CREATE POLICY "components_insert" ON public.components FOR INSERT TO authenticated WITH CHECK (true);
  END IF;
END $$;

DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename='components' AND policyname='components_update') THEN
...
ALTER TABLE public.component_types ADD COLUMN IF NOT EXISTS fields jsonb NOT NULL DEFAULT '[]'::jsonb;

-- ── audit_logs: fix SELECT policy to allow own-row reads ─────
...
-- ── warehouses RLS (idempotent) ───────────────────────────────
ALTER TABLE public.warehouses ENABLE ROW LEVEL SECURITY;

DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename='warehouses' AND policyname='warehouses_read') THEN
    CREATE POLICY "warehouses_read" ON public.warehouses FOR SELECT TO authenticated USING (true);
  END IF;
END $$;

DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename='warehouses' AND policyname='warehouses_admin_write') THEN
...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename='customers' AND policyname='customers_admin') THEN
